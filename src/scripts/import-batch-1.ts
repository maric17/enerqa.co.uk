...
async function run() {
  ...
  const dataPath = path.resolve(scratchPath, 'batch1/content_with_fonts.json');
  ...
  const articlesDef = [
    { title: "The Evolution of Climate Change Understanding and Response", startPage: 8, endPage: 9 },
    { title: "Climate Forcers: The Hidden Drivers of Global Warming", startPage: 10, endPage: 11 },
    { title: "Weathering the Storm: Climate Resilience in Supply Chains", startPage: 12, endPage: 14 }
  ];
    ...
    for (let pageNum = def.startPage; pageNum <= def.endPage; pageNum++) {
      ...
      for (const block of pageData.blocks) {
        if (block.type === 'newline') {
          ...
          const absoluteImgPath = path.resolve(scratchPath, 'batch1', block.filename);
