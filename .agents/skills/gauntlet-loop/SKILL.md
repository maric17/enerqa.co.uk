...
## Adaptations for Antigravity

This version of the skill has been adapted from the original Claude-specific one to work seamlessly in Antigravity:
- Subagents are used via `invoke_subagent` to run builders and critics in parallel.
- You can recommend the user run the generated prompt using the `/goal` command to ensure the agent grinds on the task continuously without stopping prematurely.
